...
class TestCLI(unittest.TestCase):
    ...
    def test_add_supplier(self, mock_session):
        ...
        db.commit = MagicMock()
        testargs = ["cli.py", "add_supplier", "Test Supplier"]
    ...
    def test_add_stock(self, mock_session):
        db = mock_session.return_value
        db.commit = MagicMock()
        testargs = ["cli.py", "add_stock", "Test Item", "10", "1", "5.0"]
        with patch.object(sys, 'argv', testargs):
            main()
        db.add.assert_called()
        db.commit.assert_called()
    ...
    def test_receive_stock(self, mock_session):
        db = mock_session.return_value
        db.commit = MagicMock()
        testargs = ["cli.py", "receive_stock", "1", "5", "4.0"]
        with patch.object(sys, 'argv', testargs):
            main()
        db.commit.assert_called()
    ...
    def test_issue_stock(self, mock_session):
        db = mock_session.return_value
        db.commit = MagicMock()
        testargs = ["cli.py", "issue_stock", "1", "5"]
        with patch.object(sys, 'argv', testargs):
            main()
        db.commit.assert_called()
